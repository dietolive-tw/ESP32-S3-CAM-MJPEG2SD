...
static void setWifiSTA() {
  ...
  WiFi.setSleep(false); // disable modem sleep - prevents AP-associated-but-ping-unresponsive hangs
...
static bool startWifi(bool firstcall = true) {
  ...
  if (netMode == 0) {
    ...
    if (!firstcall) {
      // explicit disconnect before reconnect - WiFi driver may be stuck even if AP shows association
...
static void pingTimeout(esp_ping_handle_t hdl, void *args) {
  ...
  // soft reconnect may not recover a stuck WiFi driver; force hard restart after repeated failures
  if (++pingFailCount >= PING_FAIL_LIMIT) {
    LOG_ERR("Ping failed %u times in a row, hard restarting", pingFailCount);
    doRestart("WiFi stuck after repeated ping failures");
  }
}
