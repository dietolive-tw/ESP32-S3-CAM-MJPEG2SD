...
uint8_t mqttStillSecs = 0; // MQTT still auto-publish interval (secs, 0=off)
...
#if (INCLUDE_PERIPH && INCLUDE_HASIO)
static char pan_setpos_topic[FILE_NAME_LEN];
static char tilt_setpos_topic[FILE_NAME_LEN];
static char pan_cmd_topic[FILE_NAME_LEN];
static char tilt_cmd_topic[FILE_NAME_LEN];
#endif
...
#if (INCLUDE_PERIPH && INCLUDE_HASIO)
  // subscribe dedicated set-position topics for HA cover sliders
  esp_mqtt_client_subscribe(mqtt_client, pan_setpos_topic, 1);
  esp_mqtt_client_subscribe(mqtt_client, tilt_setpos_topic, 1);
  // subscribe dedicated command topics for HA cover OPEN/CLOSE/STOP
  esp_mqtt_client_subscribe(mqtt_client, pan_cmd_topic, 1);
  esp_mqtt_client_subscribe(mqtt_client, tilt_cmd_topic, 1);
#endif
...
static void mqtt_data_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data) {
  ...
  if(strncmp(event->topic, HASIO_AVAILABILITY, event->topic_len) == 0){
...
#if (INCLUDE_PERIPH && INCLUDE_HASIO)
  else if (strncmp(event->topic, pan_setpos_topic, event->topic_len) == 0) {
    // HA cover slider sends raw 0~100; wrap as panPos=<pct> for existing handler
    char buf[32];
    snprintf(buf, sizeof(buf), "panPos=%.*s", event->data_len, (char*)event->data);
    ...
    char buf[32];
    snprintf(buf, sizeof(buf), "tiltPos=%.*s", event->data_len, (char*)event->data);
    ...
    char buf[24];
    snprintf(buf, sizeof(buf), "%.*s", event->data_len, (char*)event->data);
    LOG_INF("MQTT pan_cmd received: '%s'", buf);
    if (!strcmp(buf, "OPEN")) updateStatus("panPos", "100");
    else if (!strcmp(buf, "CLOSE")) updateStatus("panPos", "0");
    else if (!strcmp(buf, "STOP")) stepperStop(0);
    else LOG_WRN("Unknown pan_cmd: '%s'", buf);
  }
  else if (strncmp(event->topic, tilt_cmd_topic, event->topic_len) == 0) {
    // HA cover OPEN/CLOSE/STOP for tilt
    char buf[24];
    snprintf(buf, sizeof(buf), "%.*s", event->data_len, (char*)event->data);
    LOG_INF("MQTT tilt_cmd received: '%s'", buf);
    if (!strcmp(buf, "OPEN")) updateStatus("tiltPos", "100");
    else if (!strcmp(buf, "CLOSE")) updateStatus("tiltPos", "0");
    else if (!strcmp(buf, "STOP")) stepperStop(1);
    else LOG_WRN("Unknown tilt_cmd: '%s'", buf);
  }
#endif
#endif
  if(strncmp(event->topic, cmd_topic, event->topic_len) == 0){
    LOG_INF("MQTT cmd_topic received: '%.*s'", event->data_len, (char*)event->data);
...
void sendMqttImage(){
  if (!mqttConnected || !mqtt_client) return;
  uint32_t startTime = millis();
  if (!strlen(mqtt_topic_prefix)) return;
  // Ensure camera is awake (PIR gate) before requesting a frame.
  if (!pirGateRequireCam()) {
    LOG_WRN("Fail to send image (camera not ready)");
    return;
  }
  doKeepFrame = true;
...
static void mqttTask(void* parameter) { 
  ...
  uint32_t lastStill = 0;
  while (mqtt_active) {
    // wake on command notification, or every 1s for periodic still publish
    ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(1000));
    if (mqttConnected) {
      //Check if server sends a remote command
      checkForRemoteQuery();
      if (mqttTaskDelay > 0 ) vTaskDelay(mqttTaskDelay / portTICK_RATE_MS);
      // auto-publish still image to HA camera topic if enabled
      if (mqttStillSecs > 0) {
        uint32_t now = millis();
        if (now - lastStill >= (uint32_t)mqttStillSecs * 1000) {
          lastStill = now;
          sendMqttImage();
        }
      }
...
#if (INCLUDE_PERIPH && INCLUDE_HASIO)
  snprintf(pan_setpos_topic, FILE_NAME_LEN, "%scover/%s/pan_setpos", mqtt_topic_prefix, hostName);
  snprintf(tilt_setpos_topic, FILE_NAME_LEN, "%scover/%s/tilt_setpos", mqtt_topic_prefix, hostName);
  snprintf(pan_cmd_topic, FILE_NAME_LEN, "%scover/%s/pan_cmd", mqtt_topic_prefix, hostName);
  snprintf(tilt_cmd_topic, FILE_NAME_LEN, "%scover/%s/tilt_cmd", mqtt_topic_prefix, hostName);
#endif
...
#if (INCLUDE_PERIPH && INCLUDE_HASIO)
// MQTT Cover entity for PTZ stepper (pan or tilt)
// name: short id eg "pan" / "tilt"; displayName: HA visible name; posKey: state key for position
// cmd topic is shared cmd_topic; set_pos topic is dedicated <prefix>cover/<host>/<name>_setpos
void sendHasCover(const char* name, const char* displayName, const char* posKey) {
  char* p = jsonBuff;
  *p++ = '{';
  p += sprintf(p, "\"name\":\"%s\",", displayName);
  p += sprintf(p, "\"uniq_id\":\"%s_%012llX\",", name, ESP.getEfuseMac());
  p += sprintf(p, "\"ic\":\"%s\",", "mdi:arrow-up-down");
  // position state topic (0~100) published by sendMqttHasState
  p += sprintf(p, "\"pos_t\":\"%ssensor/%s/%s\",", mqtt_topic_prefix, hostName, posKey);
  // dedicated set-position topic: slider sends raw 0~100
  p += sprintf(p, "\"set_pos_t\":\"%scover/%s/%s_setpos\",", mqtt_topic_prefix, hostName, name);
  // open/close/stop via shared cmd topic using existing panPos/tiltPos/stepperPanStop/stepperTiltStop keys
  // command key is derived from name (pan->panPos, tilt->tiltPos), NOT posKey (pan_pos/tilt_pos
  // which is only the position STATE topic key and not handled by updateAppStatus)
  // dedicated command topic per cover: HA sends OPEN/CLOSE/STOP here
  p += sprintf(p, "\"cmd_t\":\"%scover/%s/%s_cmd\",", mqtt_topic_prefix, hostName, name);
  // payloads are default OPEN/CLOSE/STOP (no prefix needed since topic is dedicated)
  p += sprintf(p, "\"pl_open\":\"OPEN\",");
  p += sprintf(p, "\"pl_close\":\"CLOSE\",");
  p += sprintf(p, "\"pl_stop\":\"STOP\",");
  p += sprintf(p, "\"pos_open\":100,");
  p += sprintf(p, "\"pos_closed\":0,");
  p += sprintf(p, "\"opt\":false,");
  // availability
  p += sprintf(p, "\"avty_t\":\"%ssensor/%s/%s\",", mqtt_topic_prefix, hostName, "lwt");
  p += sprintf(p, "\"pl_avail\":\"%s\",", "online");
  p += sprintf(p, "\"pl_not_avail\":\"%s\",", "offline");
  ...
  *p = 0;
  char suffix[FILE_NAME_LEN];
  sprintf(suffix, "%s/config", name);
  mqttPublishPath(suffix, jsonBuff, "cover");
}
#endif
...
#if (INCLUDE_PERIPH && INCLUDE_HASIO)
  // PTZ steppers as HA cover entities (slider 0~100%)
  sendHasCover("pan", "Pan", "pan_pos");
  sendHasCover("tilt", "Tilt", "tilt_pos");
#endif
...
#if (INCLUDE_PERIPH && INCLUDE_HASIO)
  // publish PTZ stepper positions (0~100) for HA cover feedback
  sprintf(p, "%u", stepperGetPosition(0));
  mqttPublishPath("pan_pos", p);
  sprintf(p, "%u", stepperGetPosition(1));
  mqttPublishPath("tilt_pos", p);
#endif
  mqttPublishPath("free_heap", p);
  sprintf(p, "%s", fmtSize(ESP.getFreePsram()) );
  mqttPublishPath("free_psram", p);
  sprintf(p, "%s", fmtSize(storageTotalBytes() - storageUsedBytes()) );
