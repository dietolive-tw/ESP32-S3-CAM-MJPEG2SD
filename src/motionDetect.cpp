...
bool panCalibrated = false;        // pan has both ends recorded
bool tiltCalibrated = false;       // tilt has both ends recorded
...
bool checkMotion(camera_fb_t* fb, bool motionStatus, bool lightLevelOnly) {
  ...
  // accumulate centroid of changed pixels for object tracking
  int32_t sumX = 0, sumY = 0, centroidCount = 0;
  for (size_t i = 0; i < resizeDimLen; i += colorDepth) {
    ...
    if (abs((int)currPix - (int)prevPix) > detectChangeThreshold) {
      if (i > startPixel && i < endPixel) {
        ...
        // accumulate centroid position (only in active bands)
        size_t pixIdx = i / colorDepth;
        sumX += (int32_t)(pixIdx % RESIZE_DIM);
        sumY += (int32_t)(pixIdx / RESIZE_DIM);
        centroidCount++;
      }
  ...
  // calculate motion centroid as 0.0~1.0 (center of image = 0.5), or -1.0 if no motion
  // only set valid centroid when changeCount exceeds moveThreshold to avoid
  // tracking noise; otherwise invalidate so trackMotionObject() holds position
  if (changeCount > moveThreshold && centroidCount > 0) {
    motionCentroidX = (float)sumX / centroidCount / (float)(RESIZE_DIM - 1);
    motionCentroidY = (float)sumY / centroidCount / (float)(RESIZE_DIM - 1);
  } else {
    motionCentroidX = -1.0;
    motionCentroidY = -1.0;
  }
