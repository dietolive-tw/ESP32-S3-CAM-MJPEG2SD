...
static boolean processFrame() {
  ...
  if (doMonitor(doRecording ? isCapturing : dbgMotion ? false : true)) {
    ...
    if (pirTriggered) {
      ...
      // log PIR detect at most once every 5s to avoid spam (RCWL-0516 hold time)
      if (millis() - lastPirLog > 5000) {
        LOG_INF("PIR detect");
        lastPirLog = millis();
      }
    }
  ...
  // Do not attempt to capture when no SD card is present, even if forceRecord
  // or motion flags are somehow set (eg. stale state from prior session).
  isCapturing = sdCardPresent && (haveMotion | forceRecord);
  if (isCapturing && !prevCapture) {
    ...
    const char* src = reasonId == 0 ? "Button" : reasonId == 1 ? "motion detect" : reasonId == 2 ? "PIR" : "Accelerometer";
    if (!dashCamOn) LOG_ALT("motion detected start record (%s)", src);
#if INCLUDE_MQTT
    if (mqtt_active) {
...
bool prepCam() {
  ...
  uint8_t retries = 5;
  delay(500); // let camera sensor stabilize after power-up before first attempt
  while (retries && err != ESP_OK) {
    err = esp_camera_init(&config);
    if (err == ESP_OK) err = changeXCLK(config);
    if (err != ESP_OK) {
      LOG_WRN("Camera init attempt %u failed: 0x%X:%s", 6 - retries, err, espErrMsg(err));
  ...
  uint16_t PID = 0;
  if (err != ESP_OK) LOG_ERR("Camera init failed after retries: 0x%X:%s", err, espErrMsg(err));
