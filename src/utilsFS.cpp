...
// Runtime storage selector: points to SD_MMC when the card mounts at boot,
// otherwise falls back to LittleFS so the app still boots for streaming only.
#if !defined(NO_SD) && !CONFIG_IDF_TARGET_ESP32C3 && !CONFIG_IDF_TARGET_ESP32S2
fs::FS* storagePtr = &LittleFS;
#endif
bool sdCardPresent = false;

// Helpers that dispatch to the active storage's subclass-specific methods
// (fs::FS base class lacks totalBytes/usedBytes/end). Callers that need
// these values should use storageXxx() instead of STORAGE.xxx() so the
// runtime-pointer STORAGE macro works for both SD_MMC and LittleFS.
uint64_t storageTotalBytes() {
#if !defined(NO_SD) && !CONFIG_IDF_TARGET_ESP32C3 && !CONFIG_IDF_TARGET_ESP32S2
  return sdCardPresent ? SD_MMC.totalBytes() : LittleFS.totalBytes();
#else
...
#if !defined(NO_SD) && !CONFIG_IDF_TARGET_ESP32C3 && !CONFIG_IDF_TARGET_ESP32S2
  return sdCardPresent ? SD_MMC.usedBytes() : LittleFS.usedBytes();
#else
...
static void listFolder(const char* rootDir) { 
  ...
  char totalBytes[20];
  strcpy(totalBytes, fmtSize(storageTotalBytes()));
  LOG_INF("%s: %s used of %s", fsTypes[thisFS], fmtSize(storageUsedBytes()), totalBytes);
}
...
#if (!CONFIG_IDF_TARGET_ESP32C3 && !CONFIG_IDF_TARGET_ESP32S2)
  #if !defined(NO_SD)
  // SD_MMC is the preferred storage; try to mount it first
  thisFS = SDMMC;
  res = prepSD_MMC();
  if (res) {
    storagePtr = &SD_MMC;
    ...
    // SD card not present: fall back to LittleFS so the app still boots
    // for network streaming only. Recording stays disabled in this mode.
    LOG_WRN("SD card not mounted, falling back to LittleFS (recording disabled)");
    thisFS = LITTLEFS;
    ...
    if (res) {
      ...
      sdCardPresent = false;
      listFolder(DATA_DIR);
    } else snprintf(startupFailure, SF_LEN, STARTUP_FAIL "Failed to mount LittleFS");
  }
  debugMemory("startStorage");
  return res;
  #endif
#endif
  // One of SPIFFS or LittleFS (NO_SD path or non-SDMC targets)
  if (thisFS == TBD) {
#ifdef _SPIFFS_H_
...
bool checkFreeStorage() { 
  ...
  bool res = false;
  size_t freeSize = (size_t)((storageTotalBytes() - storageUsedBytes()) / ONEMEG);
  if (!sdFreeSpaceMode && freeSize < sdMinCardFreeSpace) 
    ...
    while (freeSize < sdMinCardFreeSpace) {
      ...
      freeSize = (size_t)((storageTotalBytes() - storageUsedBytes()) / ONEMEG);
    }
    LOG_INF("Storage free space: %s", fmtSize(storageTotalBytes() - storageUsedBytes()));
    res = true;
  }
  return res;
} 
