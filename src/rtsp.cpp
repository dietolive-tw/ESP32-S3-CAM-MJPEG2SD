...
static void sendRTSPVideo(void* p) {
  ...
  bool rtspClientActive = false;
  while (true) {
    if (frameSemaphore[taskNum] != NULL) {
      if (xSemaphoreTake(frameSemaphore[taskNum], pdMS_TO_TICKS(MAX_FRAME_WAIT)) == pdTRUE) {
        bool hasClient = rtspServer.readyToSendFrame();
#if INCLUDE_PERIPH
        // Wake camera when an RTSP client connects; release when it disconnects.
        if (hasClient && !rtspClientActive) pirGateRequireCam();
        else if (!hasClient && rtspClientActive) pirGateReleaseCam();
#endif
        rtspClientActive = hasClient;
        if (streamBufferSize[taskNum] && hasClient) {
