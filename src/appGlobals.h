...
#ifndef INCLUDE_PERIPH
#define INCLUDE_PERIPH false  // peripherals.cpp (servos, PIR, led etc) - override with -DINCLUDE_PERIPH=1
#endif
...
// Storage is selected at runtime: SD_MMC if the card mounts at boot,
// otherwise LittleFS is used so the app still boots for streaming only.
// Recording-related features are disabled when SD card is not present.
#ifdef NO_SD
#define STORAGE LittleFS
#else
extern fs::FS* storagePtr;
#define STORAGE (*storagePtr)
#endif
extern bool sdCardPresent; // true when SD card mounted at boot
// Helpers that dispatch to the active storage subclass (fs::FS lacks totalBytes/usedBytes/end)
...
int32_t stepperGetSteps(uint8_t idx);     // raw step count (signed)
void stepperSetSteps(uint8_t idx, int32_t steps); // set raw step count
void stopPlaying();
...
extern float motionCentroidX; // motion centroid X (0.0~1.0, -1.0 if none)
...
extern bool panCalibrated;      // pan has both ends recorded
extern bool tiltCalibrated;     // tilt has both ends recorded
...
// auxiliary lights on PCF8575 (white, red, IR) - manual web control only
#if defined(PCF8575_ADDR)
extern int auxLightPin[3];  // PCF8575 pin numbers (0-15)
extern bool auxLightOn[3];  // current on/off state
#endif
