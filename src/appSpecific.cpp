...
bool updateAppStatus(const char* variable, const char* value, bool fromUser) {
  ...
  // Reject recording-related commands when no SD card is present
  if (!sdCardPresent && (
      !strcmp(variable, "record") || !strcmp(variable, "forceRecord") ||
      !strcmp(variable, "dbgMotion") || !strcmp(variable, "timeLapseOn") ||
      !strcmp(variable, "dashCamOn") || !strcmp(variable, "enableMotion"))) {
    LOG_WRN("Ignored '%s' setting: no SD card present", variable);
    return true;
  }
#ifndef AUXILIARY
    ...
    if (pirGate) {
      useMotion = true;
      trackMotion = false;
      // PIR gating implicitly enables pirUse (needed to attach interrupt).
      pirUse = true;
      if (pirPin) pinMode(pirPin, INPUT_PULLDOWN);
      // Initialize pirGate task and attach PIR interrupt (may be enabled at runtime
      // after boot, so prepPIR() didn't attach the interrupt).
      pirGateSetup();
  ...
  else if (!strcmp(variable, "pirGatePostRecSecs")) pirGatePostRecSecs = (uint16_t)intVal;
  else if (!strcmp(variable, "pirGateBootIdleSecs")) pirGateBootIdleSecs = (uint16_t)intVal;
  ...
  else if (!strcmp(variable, "auxLight0")) setAuxLight(0, (bool)intVal);
  else if (!strcmp(variable, "auxLight1")) setAuxLight(1, (bool)intVal);
  else if (!strcmp(variable, "auxLight2")) setAuxLight(2, (bool)intVal);
  else if (!strcmp(variable, "auxLightPin0")) auxLightPin[0] = intVal;
  else if (!strcmp(variable, "auxLightPin1")) auxLightPin[1] = intVal;
  else if (!strcmp(variable, "auxLightPin2")) auxLightPin[2] = intVal;
#endif
  ...
  // Both independent ULN2003 steppers (pan & tilt) - available whenever peripherals enabled
  else if (!strcmp(variable, "stepIN1pin")) setStepperPin(0, (uint8_t)intVal, 0);
  else if (!strcmp(variable, "stepIN2pin")) setStepperPin(0, (uint8_t)intVal, 1);
  else if (!strcmp(variable, "stepIN3pin")) setStepperPin(0, (uint8_t)intVal, 2);
  else if (!strcmp(variable, "stepIN4pin")) setStepperPin(0, (uint8_t)intVal, 3);
  else if (!strcmp(variable, "step2IN1pin")) setStepperPin(1, (uint8_t)intVal, 0);
  else if (!strcmp(variable, "step2IN2pin")) setStepperPin(1, (uint8_t)intVal, 1);
  else if (!strcmp(variable, "step2IN3pin")) setStepperPin(1, (uint8_t)intVal, 2);
  else if (!strcmp(variable, "step2IN4pin")) setStepperPin(1, (uint8_t)intVal, 3);
  ...
  // value format: "<steps>,<rpm>,<clockwise>" eg "512,10,1"
    ...
    int steps; float rpm; int cw;
    if (sscanf(value, "%d,%f,%d", &steps, &rpm, &cw) == 3)
      stepperRun(0, rpm, (float)steps / stepperStepsPerRev(BYJ_48), cw ? true : false, BYJ_48);
    else LOG_WRN("stepperPan needs steps,rpm,clockwise");
  }
  else if (!strcmp(variable, "stepperTilt")) {
    int steps; float rpm; int cw;
    if (sscanf(value, "%d,%f,%d", &steps, &rpm, &cw) == 3)
      stepperRun(1, rpm, (float)steps / stepperStepsPerRev(BYJ_48), cw ? true : false, BYJ_48);
    else LOG_WRN("stepperTilt needs steps,rpm,clockwise");
  }
  else if (!strcmp(variable, "stepperPanStop")) stepperStop(0);
  else if (!strcmp(variable, "stepperTiltStop")) stepperStop(1);
  // HA cover position control: value is target percentage 0~100
  // moves shortest path to target, RPM fixed at 10
  // If axis is calibrated (panHome/tiltHome both ends recorded), the target
  // is N% of the calibrated travel range, not N% of one full revolution.
  else if (!strcmp(variable, "panPos")) {
    int pct = atoi(value);
    if (pct >= 0 && pct <= 100) {
      int32_t targetSteps;
      if (panCalibrated) {
        ...
        // fallback: N% of one revolution (NOTE: 100% == 0% due to full rev)
        targetSteps = stepperGetSteps(0) + ((int32_t)pct - (int32_t)stepperGetPosition(0))
                      * (int32_t)stepperStepsPerRev(BYJ_48) / 100;
      }
    ...
    } else LOG_WRN("panPos needs 0~100");
  }
  else if (!strcmp(variable, "tiltPos")) {
    int pct = atoi(value);
    if (pct >= 0 && pct <= 100) {
      int32_t targetSteps;
      if (tiltCalibrated) {
        ...
        targetSteps = stepperGetSteps(1) + ((int32_t)pct - (int32_t)stepperGetPosition(1))
                      * (int32_t)stepperStepsPerRev(BYJ_48) / 100;
      }
    ...
    } else LOG_WRN("tiltPos needs 0~100");
  }
  // calibrate / reset position reference (no movement)
  else if (!strcmp(variable, "panSetPos")) stepperSetPosition(0, (uint8_t)atoi(value));
  else if (!strcmp(variable, "tiltSetPos")) stepperSetPosition(1, (uint8_t)atoi(value));
  // convenience: zero both steppers to center (50% = 90°) without moving
  else if (!strcmp(variable, "stepperZero")) {
    if ((bool)intVal) {
      stepperSetPosition(0, 50);
      stepperSetPosition(1, 50);
      LOG_INF("Stepper positions zeroed to center (90°)");
      updateConfigVect("stepperZero", "0"); // reset so it doesn't persist
    }
  }
  // Manual homing calibration (no limit switches; mechanical stops only).
  // Two-stage flow per axis:
  //   1. /control?panHome=0  -> mark current raw step count as 0% end A
  //   2. user manually drives the axis to the other mechanical stop
  //   3. /control?panHome=100 -> record raw step count at this end as 100% end B
  //      After this, panPos=N moves N% of the A->B travel (not N% of one rev),
  //      so panPos=100 reaches the actual end B instead of wrapping to start.
  //   panHome=50 clears calibration (fallback to per-rev mode) and marks
  //   current position as 50%.
  else if (!strcmp(variable, "panHome")) {
    int ref = atoi(value);
    int32_t s = stepperGetSteps(0);
    if (ref == 0) {
      panCalSteps[0] = s;
      panCalibrated = false;
      stepperSetPosition(0, 0);
      LOG_INF("Pan home: end A (0%%) at steps=%d", (int)s);
    } else if (ref == 100) {
      if (panCalSteps[0] == s) { LOG_WRN("Pan home: end B same as end A, ignored"); }
      else {
        panCalSteps[1] = s;
        panCalibrated = true;
        int32_t travel = panCalSteps[1] - panCalSteps[0];
        LOG_INF("Pan home: end B (100%%) at steps=%d, travel=%d, calibrated", (int)s, (int)travel);
      }
    } else if (ref == 50) {
      panCalibrated = false;
      stepperSetPosition(0, 50);
      LOG_INF("Pan calibration cleared, set as center (90°) at steps=%d", (int)s);
    } else LOG_WRN("panHome needs 0, 50, or 100");
  }
  else if (!strcmp(variable, "tiltHome")) {
    int ref = atoi(value);
    int32_t s = stepperGetSteps(1);
    if (ref == 0) {
      tiltCalSteps[0] = s;
      tiltCalibrated = false;
      stepperSetPosition(1, 0);
      LOG_INF("Tilt home: end A (0%%) at steps=%d", (int)s);
    } else if (ref == 100) {
      if (tiltCalSteps[0] == s) { LOG_WRN("Tilt home: end B same as end A, ignored"); }
      else {
        tiltCalSteps[1] = s;
        tiltCalibrated = true;
        int32_t travel = tiltCalSteps[1] - tiltCalSteps[0];
        LOG_INF("Tilt home: end B (100%%) at steps=%d, travel=%d, calibrated", (int)s, (int)travel);
      }
    } else if (ref == 50) {
      tiltCalibrated = false;
      stepperSetPosition(1, 50);
      LOG_INF("Tilt calibration cleared, set as center (90°) at steps=%d", (int)s);
    } else LOG_WRN("tiltHome needs 0, 50, or 100");
  }
  // Direct travel calibration: skip the manual drive to end B.
  // If panHome=0 was previously called, uses that end A as reference;
  // otherwise treats current position as end A.
  // Positive steps = end B is cw from end A; negative = ccw.
  else if (!strcmp(variable, "panTravel")) {
    int32_t travel = (int32_t)atol(value);
    if (travel == 0) { LOG_WRN("panTravel needs non-zero step count (signed: +cw / -ccw)"); }
    else {
      if (!panCalibrated && panCalSteps[0] == 0 && stepperGetSteps(0) != 0) {
        // panHome=0 was never called; use current position as end A
        panCalSteps[0] = stepperGetSteps(0);
        stepperSetPosition(0, 0);
      }
      panCalSteps[1] = panCalSteps[0] + travel;
      panCalibrated = true;
      LOG_INF("Pan travel=%d set: end A=%d, end B=%d, calibrated", (int)travel, (int)panCalSteps[0], (int)panCalSteps[1]);
    }
  }
  else if (!strcmp(variable, "tiltTravel")) {
    int32_t travel = (int32_t)atol(value);
    if (travel == 0) { LOG_WRN("tiltTravel needs non-zero step count (signed: +cw / -ccw)"); }
    else {
      if (!tiltCalibrated && tiltCalSteps[0] == 0 && stepperGetSteps(1) != 0) {
        tiltCalSteps[0] = stepperGetSteps(1);
        stepperSetPosition(1, 0);
      }
      tiltCalSteps[1] = tiltCalSteps[0] + travel;
      tiltCalibrated = true;
      LOG_INF("Tilt travel=%d set: end A=%d, end B=%d, calibrated", (int)travel, (int)tiltCalSteps[0], (int)tiltCalSteps[1]);
    }
  }
...
#if INCLUDE_PERIPH
    if (!pirGateRequireCam()) {
      httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Camera waking up, retry");
      return ESP_OK;
    }
#endif
    doKeepFrame = true;
...
static bool setPeripheral(char cmd, int controlVal, bool fromUart) {
  ...
  switch (cmd) {
    ...
    case 'P':
      // camera pan servo
      manualStepperUntilMs = millis() + 5000;
    ...
    break;
    case 'p': {
      // stepper pan: controlVal encodes steps*1000 + rpm*10 + cw (compact form)
      // steps = controlVal / 100000, rpm = (controlVal / 10) % 10000, cw = controlVal % 10
      int steps = controlVal / 100000;
      float rpm = (float)((controlVal / 10) % 10000);
      bool cw = controlVal % 10;
      if (steps && rpm > 0) stepperRun(0, rpm, (float)steps / stepperStepsPerRev(BYJ_48), cw, BYJ_48);
      else stepperStop(0);
    break; }
    case 't': {
      // stepper tilt: same compact encoding as 'p'
      int steps = controlVal / 100000;
      float rpm = (float)((controlVal / 10) % 10000);
      bool cw = controlVal % 10;
      if (steps && rpm > 0) stepperRun(1, rpm, (float)steps / stepperStepsPerRev(BYJ_48), cw, BYJ_48);
      else stepperStop(1);
    break; }
    case 'q':
      // stop stepper pan
    ...
    break;
    case 'Z':
      // zero/center stepper position: reset current position to 50% (90°) without moving motor
      // user should manually center the camera first, then trigger this to sync slider
      if (stepperUse) {
        if (stepINpins[0][0] > 0) stepperSetPosition(0, 50);
        if (stepINpins[1][0] > 0) stepperSetPosition(1, 50);
        LOG_INF("Stepper positions zeroed to center (90°)");
      }
    break;
#endif
...
char* buildAppJsonString(bool filter) {
  ...
  // report stepper position as camPan/camTilt angle so web UI slider reflects actual position
  if (stepperUse) {
    p += sprintf(p, "\"camPan\":\"%d\",", (int)stepperGetPosition(0) * 180 / 100);
    p += sprintf(p, "\"camTilt\":\"%d\",", (int)stepperGetPosition(1) * 180 / 100);
    p += sprintf(p, "\"trackMotion\":\"%d\",", trackMotion ? 1 : 0);
  }
 #if INCLUDE_AUDIO
 ...
 #endif
 #if defined(PCF8575_ADDR)
   p += sprintf(p, "\"auxLight0\":\"%d\",", auxLightOn[0] ? 1 : 0);
   p += sprintf(p, "\"auxLight1\":\"%d\",", auxLightOn[1] ? 1 : 0);
   p += sprintf(p, "\"auxLight2\":\"%d\",", auxLightOn[2] ? 1 : 0);
 #endif
#endif
#if INCLUDE_MCPWM
  ...
  // Extend info
#ifndef AUXILIARY
  p += sprintf(p, "\"sdCardPresent\":\"%d\",", sdCardPresent ? 1 : 0);
  uint8_t cardType = 99; // not MMC
  if (sdCardPresent) cardType = SD_MMC.cardType();
  if (cardType == CARD_NONE) p += sprintf(p, "\"card\":\"%s\",", "NO card");
    ...
    if (sdCardPresent) p += sprintf(p, "\"card_size\":\"%s\",", fmtSize(SD_MMC.cardSize()));
    p += sprintf(p, "\"used_bytes\":\"%s\",", fmtSize(storageUsedBytes()));
    p += sprintf(p, "\"free_bytes\":\"%s\",", fmtSize(storageTotalBytes() - storageUsedBytes()));
    p += sprintf(p, "\"total_bytes\":\"%s\",", fmtSize(storageTotalBytes()));
  }
...
bool appDataFiles() {
  ...
  // patch common.js on SD card: fix audioContextSpkr -> audioContextMic in runMic()
  const char* jsPath = DATA_DIR "/common.js";
  if (STORAGE.exists(jsPath)) {
    File f = STORAGE.open(jsPath, FILE_READ);
    if (f) {
      size_t fSize = f.size();
      ...
      if (buf) {
        ...
        buf[fSize] = 0;
        bool patched = false;
        // find runMic function and replace audioContextSpkr only within it
        char* runMicStart = strstr((char*)buf, "async function runMic(");
        char* runMicEnd = runMicStart ? strstr(runMicStart, "function showMicLevel(") : NULL;
        if (runMicStart && runMicEnd) {
          char* p = runMicStart;
          while (p < runMicEnd) {
            p = strstr(p, "audioContextSpkr");
            if (!p || p >= runMicEnd) break;
            memcpy(p, "audioContextMic  ", 16);
            patched = true;
            p += 16;
          }
        }
        if (patched) {
          f = STORAGE.open(jsPath, FILE_WRITE);
          if (f) { f.write(buf, fSize); f.close(); LOG_INF("Patched common.js: audioContextSpkr -> audioContextMic in runMic()"); }
        }
        free(buf);
      }
    }
...
pirPin~3~3~N~Pin used for PIR
lampPin~~3~N~Pin used for Lamp
auxLightPin0~4~3~N~PCF8575 pin (0-15) for White light
auxLightPin1~5~3~N~PCF8575 pin (0-15) for Red light
auxLightPin2~6~3~N~PCF8575 pin (0-15) for IR light
...
mqttStillSecs~0~2~N~MQTT still auto publish interval (secs, 0=off)
...
stepIN1pin~1~3~N~Stepper 0 (pan) IN1 pin number
stepIN2pin~2~3~N~Stepper 0 (pan) IN2 pin number
stepIN3pin~42~3~N~Stepper 0 (pan) IN3 pin number
stepIN4pin~41~3~N~Stepper 0 (pan) IN4 pin number
step2IN1pin~45~3~N~Stepper 1 (tilt) IN1 pin number
step2IN2pin~47~3~N~Stepper 1 (tilt) IN2 pin number
step2IN3pin~21~3~N~Stepper 1 (tilt) IN3 pin number
step2IN4pin~14~3~N~Stepper 1 (tilt) IN4 pin number
stepperUse~1~3~C~Enable stepper motors
