...
static bool ampPlaying = false; // track if amp is actively outputting
...
#define MAX98357_SD_PIN 47 // MAX98357A shutdown/mute control
#endif
...
static bool setupMic() {
  ...
  if (I2Smic) {
    ...
    res = I2Sstd.begin(I2S_MODE_STD, SAMPLE_RATE, I2S_DATA_BIT_WIDTH_16BIT, I2S_SLOT_MODE_STEREO, I2S_STD_SLOT_BOTH);
  } else {
...
#if defined(MAX98357_SD_PIN)
  if (res) {
    pinMode(MAX98357_SD_PIN, OUTPUT);
    digitalWrite(MAX98357_SD_PIN, LOW); // keep muted until actual playback
  }
#endif
  return res;
}
...
static size_t espMicInput() {
  ...
  if (micUse) {
    if (I2Smic) {
      // STEREO mode: read L+R interleaved, extract left channel
      size_t stereoBytes = sampleBytes * 2;
      int16_t* stereoBuf = (int16_t*)malloc(stereoBytes);
      if (stereoBuf) {
        size_t br = I2Sstd.readBytes((char*)stereoBuf, stereoBytes);
        int samples = br / 4;
        for (int i = 0; i < samples; i++) sampleBuffer[i] = stereoBuf[i * 2];
        bytesRead = samples * sampleWidth;
        free(stereoBuf);
      }
    } else {
      bytesRead = I2Spdm.readBytes((char*)sampleBuffer, sampleBytes);
    }
...
#if defined(MAX98357_SD_PIN)
    if (!ampPlaying) { digitalWrite(MAX98357_SD_PIN, HIGH); ampPlaying = true; }
#endif
    // STEREO mode: duplicate mono to both L and R channels
    int samples = bytesRead / sampleWidth;
    int16_t* stereoBuf = (int16_t*)malloc(samples * 2 * sampleWidth);
    if (stereoBuf) {
      for (int i = 0; i < samples; i++) {
        stereoBuf[i * 2] = sampleBuffer[i];
        stereoBuf[i * 2 + 1] = sampleBuffer[i];
      }
      I2Sstd.write((uint8_t*)stereoBuf, samples * 2 * sampleWidth);
      free(stereoBuf);
    }
  }
...
void ampMute() {
#if defined(MAX98357_SD_PIN)
  digitalWrite(MAX98357_SD_PIN, LOW);
  ampPlaying = false;
#endif
}
...
void browserMicInput(uint8_t* wsMsg, size_t wsMsgLen) {
  ...
  if (wsBufferLen == 0) {
    ...
    if (!ampPlaying) { digitalWrite(MAX98357_SD_PIN, HIGH); ampPlaying = true; }
#endif
    int samples = copyLen / sizeof(int16_t);
    int16_t* stereoBuf = (int16_t*)malloc(samples * 2 * sizeof(int16_t));
    if (stereoBuf) {
      int16_t* wsPtr = (int16_t*)wsBuffer;
      for (int i = 0; i < samples; i++) {
        stereoBuf[i * 2] = wsPtr[i];
        stereoBuf[i * 2 + 1] = wsPtr[i];
      }
      size_t written = I2Sstd.write((uint8_t*)stereoBuf, samples * 2 * sizeof(int16_t));
      free(stereoBuf);
      LOG_INF("browserMic: %d samples, %d bytes written", samples, written);
    }
    wsBufferLen = 0;
  }
}    
