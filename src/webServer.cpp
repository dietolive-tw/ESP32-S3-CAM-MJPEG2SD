...
esp_err_t uploadHandler(httpd_req_t *req) {
  ...
  if (strstr(inFileName, ".bin") != NULL) {
    ...
    if (cmd == U_SPIFFS) storageEnd(); // close relevant file system
...
static bool checkWsSocketStatus() {
  ...
  if (fdWs < 0) return false;
  if (httpd_ws_get_fd_info(httpServer, fdWs) != HTTPD_WS_CLIENT_WEBSOCKET) {
    fdWs = -1;
    return false;
  }
  return true;
...
bool wsAsyncSendText(const char* wsData) {
  ...
  if (checkWsSocketStatus()) {
    ...
    if (ret != ESP_OK) {
      LOG_WRN("websocket send failed with %s", esp_err_to_name(ret));
      fdWs = -1; // invalidate to prevent crash on stale TLS session
      return false;
    }
    return true;
  } 
...
static esp_err_t wsHandler(httpd_req_t *req) {
  ...
  if (req->method == HTTP_GET) {
    ...
    if (ret == ESP_OK) {
      ...
      int failedFd = httpd_req_to_sockfd(req);
      if (failedFd == fdWs) fdWs = -1;
      httpd_sess_trigger_close(httpServer, failedFd);
    }
  }
  return ret;
}
